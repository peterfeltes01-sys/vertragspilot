...
const d = (s) => (s ? new Date(s).toISOString() : null);
...
async function main() {
  ...
  for (const kat of kategorien) {
    const existing = await db.collection("kategorien").where("name", "==", kat.name).get();
    if (existing.empty) {
      await db.collection("kategorien").add(kat);
    }
  }

  console.log("📑 Seeding Verträge (bestehende werden gelöscht)...");
  const existing = await db.collection("contracts").get();
